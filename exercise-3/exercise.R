...
# Make a variable called pokemon that has the names of your favorite pokemon
# be aware of casing! 
...
# it with the Poke API? 
  ...
  # You can call by number or by name (ie pokemon/1/ or pokemon/charizard/)
...
# What do you think is the diffrence between the two outputs above? Why are they diffrent?
  ...
  #Poke Data is essentially a list of data frames, so accessing the list will return false
  # because it isnt a data frame, but accessing a data frame from that list will return true because
  # it is a data frame
...
# first few values. Feel free to use view to look at the dataframe.
  ...
  poke.moves <- poke.data$moves %>% head()
...
# Using dplyr remove all columns except the one with your move names. 
  ...
  move.names <- select(poke.moves, move.name)
...
# indicates on a scale of 1-5 how much you like that move, with 1 being not at all and 5 being it's my favorite. 
  ...
  poke.moves <- mutate(poke.moves, move.type = c("normal", "fire", "thunder", "flying", "normal", "flying"),
...
# your pokemon's name. Be sure to check that there is a link to the sprite you chose.
 ...
 poke.img <- poke.data$sprites
  download.file(poke.img$front_default[1], destfile="png/charizard.png", mode="wb")
