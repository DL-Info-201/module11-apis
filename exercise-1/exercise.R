...
# For what years does the API have statistical data?
  time <- GET("http://data.unhcr.org/api/stats/time_series_years.json")
  content <- content(time, "text")
  json.content <- fromJSON(content)

# What is the "country code" for the "Syrian Arab Republic"?
  
  countries <- GET("http://data.unhcr.org/api/countries/list.json")
  content <- content(countries, "text")
  json <- fromJSON(content)
  country.code <- filter(json, name_en == "Syrian Arab Republic") %>% select(country_code)
  View(country.code)
...
# Hint: check out the "time series" end points
  ...
  ref.params <- list(population_type_code = "RF", country_of_residence = "USA", country_of_origin = "SYR")
  refugees <- GET("http://data.unhcr.org/api/stats/time_series_all_years.json", query=ref.params)
  refugee.data <-fromJSON(content(refugees, "text"))
...
# Hint: To compare the values, you'll need to convert the data (which is a string) to a number; try using `as.numeric()`
   ...
   total.data <- left_join(refugees.data, refugees.other, by="year") %>%
